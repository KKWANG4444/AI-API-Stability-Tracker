...
GPT-6 Sol、GPT-6 Luna 与 Claude Opus 5.5 已进入 AIFast 当前模型目录，API 模型 ID 分别为 `gpt-6-sol`、`gpt-6-luna` 和 `claude-opus-5-5`。本仓库不把模型目录当成稳定性实测结果；需要评估时，请按时间、地区、网络、请求特征和响应结构记录独立样本。
